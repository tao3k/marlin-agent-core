...
/// Tracing target for runtime-owned process lifecycle events.
pub const TARGET_RUNTIME_PROCESS: &str = "runtime.process";
...
/// Tracing field key for the runtime observation timestamp in milliseconds.
pub const FIELD_OBSERVED_AT_MS: &str = "observed_at_ms";
/// Tracing field key for an OS process identifier.
pub const FIELD_PROCESS_ID: &str = "pid";
/// Tracing field key for a runtime process event classifier.
pub const FIELD_PROCESS_EVENT: &str = "process_event";
/// Tracing field key for a runtime process handle.
pub const FIELD_PROCESS_HANDLE: &str = "process_handle";
/// Tracing field key for a runtime process kind.
pub const FIELD_PROCESS_KIND: &str = "process_kind";
/// Tracing field key for process lifecycle state.
pub const FIELD_PROCESS_STATUS: &str = "process_status";
/// Tracing field key for a runtime owner reference.
pub const FIELD_OWNER_REFERENCE: &str = "owner_reference";
/// Tracing field key for a process cleanup outcome.
pub const FIELD_CLEANUP_OUTCOME: &str = "cleanup_outcome";
/// Tracing field key for whether a cleanup outcome remains registry-visible.
pub const FIELD_RETAINED_IN_REGISTRY: &str = "retained_in_registry";
/// Tracing field key for whether cleanup needs another observation pass.
pub const FIELD_REQUIRES_FOLLOW_UP: &str = "requires_follow_up";
/// Tracing field key for whether force cleanup should be considered.
pub const FIELD_FORCE_CLEANUP_RECOMMENDED: &str = "force_cleanup_recommended";
/// Tracing field key for graceful cleanup attempt count.
pub const FIELD_CLEANUP_ATTEMPTS: &str = "cleanup_attempts";
/// Tracing field key for the last graceful cleanup attempt timestamp in milliseconds.
pub const FIELD_LAST_CLEANUP_ATTEMPT_AT_MS: &str = "last_cleanup_attempt_at_ms";
/// Tracing field key for cleanup sweep candidate count.
pub const FIELD_CLEANUP_CANDIDATE_COUNT: &str = "cleanup_candidate_count";
/// Tracing field key for stale process removals in one cleanup sweep.
pub const FIELD_REMOVED_STALE_COUNT: &str = "removed_stale_count";
/// Tracing field key for termination requests in one cleanup sweep.
pub const FIELD_TERMINATION_REQUESTED_COUNT: &str = "termination_requested_count";
/// Tracing field key for termination failures in one cleanup sweep.
pub const FIELD_TERMINATION_FAILED_COUNT: &str = "termination_failed_count";

/// Runtime process event value for a newly tracked child process.
pub const PROCESS_EVENT_TRACKED: &str = "tracked";
/// Runtime process event value for a cleanup sweep summary.
pub const PROCESS_EVENT_CLEANUP_SWEEP: &str = "cleanup_sweep";
/// Runtime process event value for a finished child process.
pub const PROCESS_EVENT_FINISHED: &str = "finished";
/// Runtime process event value for a failed child process.
pub const PROCESS_EVENT_FAILED: &str = "failed";
/// Runtime process event value for an orphaned child process.
pub const PROCESS_EVENT_ORPHANED: &str = "orphaned";
/// Runtime process event value for a cleanup-requested child process.
pub const PROCESS_EVENT_CLEANUP_REQUESTED: &str = "cleanup_requested";
/// Runtime process event value for a stale process removed during cleanup.
pub const PROCESS_EVENT_REMOVED_STALE: &str = "removed_stale";
/// Runtime process event value for a process that accepted termination.
pub const PROCESS_EVENT_TERMINATION_REQUESTED: &str = "termination_requested";
/// Runtime process event value for a process that refused termination.
pub const PROCESS_EVENT_TERMINATION_FAILED: &str = "termination_failed";
